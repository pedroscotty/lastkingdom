...
"""
    ...
    Usage Examples:
        ...
        <dir>
            ...
            <toonnova>main/updates/0</toonnova>
        </dir>

        <dir>
            <title>Latest Releases (Full List)</title>
            <toonnova>wcdaily-updates</toonnova>
        </dir>

        <dir>
            <title>Popular Series (Short List)</title>
            <toonnova>main/popular_series/0</toonnova>
        </dir>

        <dir>
            <title>Popular Series (Full List)</title>
            <toonnova>popular-cartoon</toonnova>
        </dir>

        <dir>
            <title>Cartoons</title>
            <toonnova>category/cartoon</toonnova>
        </dir>

        <dir>
            <title>Movies</title>
            <toonnova>category/movies</toonnova>
        </dir>

        <dir>
            <title>Search Movies</title>
            <toonnova>wcsearch</toonnova>
        </dir>

        <dir>
            <title>All 101 Dalmatians Movies</title>
            <toonnova>wcsearch/101 dalmatians</toonnova>
        </dir>
...
class ToonNova(Plugin):
    name = "toonnova"

    def process_item(self, item_xml):
        if "<toonnova>" in item_xml:
            item = JenItem(item_xml)
            if "TNepisode/" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "TNEpisodes",
                    'url': item.get("toonnova", ""),
                    'folder': True,
                    'imdb': "0",
                    'content': "files",
                    'season': "0",
            ...
            elif "TNsearch" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
            ...
            elif "list-videos/" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
            ...
            elif "direct/" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
            ...
            elif "main/" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
            ...
            elif "popular-cartoon" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
            ...
            elif "TNdaily-updates" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
            ...
            elif "category/" in item.get("toonnova", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
...
@route(mode='TNMain', args=["url"])
def get_TNmainstream(subid):
    xml = ""
...
@route(mode='TNPopular', args=["url"])
def get_TNpopular(url):
    url = urlparse.urljoin('http://www.toonova.net/', url)
...
@route(mode='TNDaily', args=["url"])
def get_TNdaily(url):
    url = url.replace('TNdaily-', '') # Strip our episode tag off.
    url = urlparse.urljoin('http://www.toonova.net/', url)
    ...
    if not xml:
        ...
        try:
            ...
            if len(pagination) > 0:
                list_items = dom_parser.parseDOM(pagination, 'li')
                next_li = list_items[(len(list_items)-1)]
                next_url = 'TNdaily-updates/%s' % (re.compile('href="http://www.toonova.net/updates/(.+?)"',re.DOTALL).findall(next_li)[0])
                xml += "<dir>"\
                       "    <title>Next Page >></title>"\
                       "    <toonnova>%s</toonnova>"\
                       "    <thumbnail>%s</thumbnail>"\
...
@route(mode='TNEpisodes', args=["url"])
def get_wcepisodes(url):
    url = url.replace('TNepisode/', '') # Strip our episode tag off.
    url = urlparse.urljoin('http://www.toonova.net/', url)
    ...
    if not xml:
        ...
        try:
            ...
            for entry in lists:
                ...
                xml += "<item>"\
                       "    <title>%s</title>"\
                       "    <toonnova>list-videos/%s</toonnova>"\
                       "    <thumbnail>%s</thumbnail>"\
                       "    <summary>%s</summary>"\
...
@route(mode='TNSearch', args=["url"])
def get_wcsearch(url):
    xml = ""
    url = url.replace('TNsearch/', '') # Strip our search tag off when used with keywords in the xml
    url = url.replace('TNsearch', '') # Catch plain case, for when overall search is used.
    ...
    if search == None or search == "":
        xml += "<item>"\
               ...
               "    <link>plugin://plugin.video.jew/?mode=section_item</link>"\
               "    <thumbnail>%s</thumbnail>"\
               "</item>" % (addon_icon)
        jenlist = JenList(xml)
        display_list(jenlist.get_list(), jenlist.get_content_type())
...
@route(mode='TNListVideos', args=["url"])
def get_TNlistvideos(url):
...
@route(mode='TNPlayVideo', args=["url"])
def get_TNplayvideo(url):
